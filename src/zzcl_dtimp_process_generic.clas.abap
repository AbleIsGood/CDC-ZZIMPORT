CLASS zzcl_dtimp_process_generic DEFINITION
  ...
  PROTECTED SECTION.
  PRIVATE SECTION.
    DATA: ms_configuration TYPE STRUCTURE FOR READ RESULT zr_zt_dtimp_conf.
    DATA: ms_file TYPE STRUCTURE FOR READ RESULT zr_zt_dtimp_files.
...
*          out->write( |sy-batch = abap_false | ).
  ...
  METHOD get_batch_import_configuration.
    READ ENTITY zr_zt_dtimp_conf ALL FIELDS WITH VALUE #( (  %key-uuid = ms_file-uuidconf ) )
...
*        RAISE EXCEPTION TYPE cx_bali_runtime.
  ...
  METHOD get_file_content.
    READ ENTITY zr_zt_dtimp_files ALL FIELDS WITH VALUE #( (  %key-uuid = p_uuid ) )
...
*          add_text_to_app_log_or_console( |{ <fs_s_e>-('OrderID') }   { <fs_s_e>-('DeliveryDate') }  { <fs_s_e>-('OrderQuantity') }| ).
  ...
  METHOD save_job_info.
    IF sy-batch = abap_true.
      ...
      MODIFY ENTITY zr_zt_dtimp_files UPDATE FIELDS ( jobcount jobname loghandle ) WITH VALUE #( ( jobcount = jobcount jobname = jobname loghandle = log_handle uuid = uuid ) ).
      COMMIT ENTITIES.
    ENDIF.
  ENDMETHOD.
ENDCLASS.
