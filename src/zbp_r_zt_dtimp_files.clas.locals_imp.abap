CLASS lsc_zr_zt_dtimp_files DEFINITION INHERITING FROM cl_abap_behavior_saver.
...
CLASS lsc_zr_zt_dtimp_files IMPLEMENTATION.
...
*            DATA(error_message) = job_scheduling_error->get_text( ).
         ...
         " catch CX_SY_REF_IS_INITIAL into data(root_exception).
...
ENDCLASS.
CLASS LHC_ZR_ZT_DTIMP_FILES DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      GET_GLOBAL_AUTHORIZATIONS FOR GLOBAL AUTHORIZATION
        IMPORTING
           REQUEST requested_authorizations FOR ZR_ZT_DTIMP_FILES
        RESULT result,
      get_instance_features FOR INSTANCE FEATURES
        IMPORTING keys REQUEST requested_features FOR Files RESULT result.
ENDCLASS.

CLASS LHC_ZR_ZT_DTIMP_FILES IMPLEMENTATION.
  METHOD GET_GLOBAL_AUTHORIZATIONS.
  ENDMETHOD.
   METHOD get_instance_features.
    READ ENTITIES OF zr_zt_dtimp_files IN LOCAL MODE
        ENTITY Files
     FIELDS (  uuid JobName )
